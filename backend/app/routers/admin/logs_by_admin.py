...
router = APIRouter(prefix="/api/admin", tags=["Logs by Admin"])


@router.get("/logs")
def get_logs(db: Session = Depends(get_db), current_user=Depends(require_admin)):
...
def get_system_updates(
    ...
    forecast_log = (
        db.query(AuditLog)
        .filter(AuditLog.entity == "FORECAST")
        .order_by(AuditLog.created_at.desc())
        .first()
    )
    ...
    return {
        "events_update": {
            ...
            "imported": events_log.new_data.get("imported")
            if events_log and events_log.new_data
            else 0,
            "skipped": events_log.new_data.get("skipped")
            if events_log and events_log.new_data
            else 0,
        },
        "forecast_update": {
            ...
            "model": forecast_log.new_data.get("model")
            if forecast_log and forecast_log.new_data
            else None,
            "records_generated": forecast_log.new_data.get("records_generated")
            if forecast_log and forecast_log.new_data
            else 0,
        },
    }
