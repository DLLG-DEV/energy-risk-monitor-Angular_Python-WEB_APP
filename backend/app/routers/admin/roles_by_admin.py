...
router = APIRouter(prefix="/api/admin", tags=["Roles by Admin"])
...
def get_roles_list(db: Session = Depends(get_db), current_user=Depends(require_admin)):
...
@router.post("/roles")
def create_role(
    data: RoleCreate, db: Session = Depends(get_db), current_user=Depends(require_admin)
):
    ...
    new_role = Role(name=data.name, modules=data.modules)

    db.add(new_role)
    db.commit()
    db.refresh(new_role)

    new_data = {"name": new_role.name, "modules": new_role.modules}
    ...
    return {"message": "Rol creado correctamente", "id": new_role.id}
...
def update_role(
    ...
    old_data = {"name": role.name, "modules": role.modules}
    ...
    new_data = {"name": role.name, "modules": role.modules}
