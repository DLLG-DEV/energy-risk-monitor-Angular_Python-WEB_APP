...
export class UpdateSistemData implements OnInit {
  ...
  loadUpdates() {
    this.adminService.getSystemUpdates().subscribe({
  ...
  formatDate(date: string) {
    ...
    return new Date(cleanDate).toLocaleString('es-MX', {
      dateStyle: 'medium',
      timeStyle: 'short',
    });
  }
  ...
  importNASA() {
    if (!this.importAmount || !this.importUnit) {
    ...
    this.adminService.importEvents(this.importAmount, this.importUnit).subscribe({
      next: (response) => {
        this.messageService.add({
          severity: 'success',
          summary: 'Importación completada',
          detail: `NASA EONET actualizado correctamente`,
        });
        this.refresh_logs();
        this.loadUpdates();
      ...
      error: (err) => {
        this.messageService.add({
          ...
          detail: err.error?.detail || 'No fue posible actualizar los datos NASA',
        });
        this.loadingImport = false;
      },
    });
  ...
  generateForecast() {
    ...
    this.fore_service.generateForecast().subscribe({
      next: (response) => {
        this.refresh_logs();
        this.loadUpdates();
      ...
      error: (err) => {
        ...
        this.messageService.add({
          ...
          detail: err.error?.detail || 'No fue posible generar el modelo predictivo',
        });
