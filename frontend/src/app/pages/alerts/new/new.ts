...
export class NewAlert implements OnInit {
  ...
  isFormValid(): boolean {
    return !!(this.form.country && this.form.category && this.form.periodicity);
  }
  ...
  loadCatCategories() {
    ...
    this.alarmService.getcat_categories().subscribe({
      next: (data) => {
  ...
  save() {
    ...
    const request = this.selectedId
      ? this.alarmService.updateAlarm(this.selectedId, data)
      : this.alarmService.createAlarm(data);
  ...
  searchCountry() {
    const value = this.countrySearch.trim();
    ...
    this.alarmService.searchCountries(value).subscribe({
      next: (data) => {
        this.countries = data;
